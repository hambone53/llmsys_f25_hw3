...
def cross_entropy_loss(out, y):
    ...
    y_ones = minitorch.ones(y.shape)

    # 2. Compute log softmax of out and (ones - out)
    out_logsoftmax = minitorch.nn.logsoftmax(out, 0)
    out_ones = minitorch.ones(out.shape)
    out_ones_logsoftmax = minitorch.nn.logsoftmax((out_ones - out), 0)

    # 3. Calculate binary cross entropy and take mean
    # HINT: Use minitorch.tensor_functions.ones, minitorch.nn.logsoftmax
    losses = y * out_logsoftmax + (y_ones - y) * out_ones_logsoftmax
...
class Linear(minitorch.Module):
    ...
    def forward(self, x):
        ...
        out = x.__matmul__(weights)
        out = out.view(batch,self.out_size)
        out = out.__add__(self.bias.value)
        return out
...
class Network(minitorch.Module):
    ...
    def __init__(
        ...
        self.linear_layer_1 = Linear(embedding_dim, hidden_dim)
        self.linear_layer_2 = Linear(hidden_dim,1)
    ...
    def train(
        ...
        for epoch in range(1, max_epochs + 1):
            ...
            for batch_num, example_num in enumerate(
                ...
                # batch_num gets the index (0, 1, 2, ...) and example_num is the starting index value from range (1 100 ...)
                x_batch = X_train[example_num:example_num+batch_size]
                y_batch = y_train[example_num:example_num+batch_size]
                x = minitorch.tensor(x_batch)
                y = minitorch.tensor(y_batch)
